"""DGLifting variant: UDE -> DLST in sequence.

The depth-joint token coming out of ``RGBD_Extraction`` is first refined by the
original Uncertainty-aware Depth Enhancer (UDE) from ``DGLifting.py`` (depth +
uncertainty regression -> uncer-weighted self-attention), and then fed into the
Depth-Layer Sorting Transformer (DLST) from ``DGLifting_dlst.py``. This is in
contrast to ``DGLifting_dlst.py`` where DLST *replaces* UDE.

Output signature matches DLST (``x, rel_depth, layer_assign``) so it plugs into
the existing ``DepthGuidedPoseDLST`` training path (ordering loss + diagnostics).
"""
...
class DGLifting(nn.Module):
    """UDE -> DLST sequential variant of DGLifting.

    The depth-joint token from RGBD_Extraction is first refined by UDE
    (uncertainty-aware depth enhancer), then passed through DLST
    (depth-layer sorting transformer). Both UDE's (coarse_depth, uncer) and
    DLST's (rel_depth, layer_assign) are returned so the baseline BNN loss
    on UDE plus the DLST ordering loss can both be applied.
    ...
    def forward(self, keypoints_2d, ref, depth_images, features_list_hr):
        ...
        # ---- UDE: refine the depth-joint token with uncertainty-aware self-attention ----
        x_depth = x[:, -1]
        ...
        # ---- DLST on top of UDE-refined depth tokens ----
