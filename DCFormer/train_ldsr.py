...
from mvn.models.DGPose_ldsr import DepthGuidedPoseLDSR
from mvn.models.loss import MPJPE, KeypointsMSELoss, KeypointsMSESmoothLoss, KeypointsMAELoss, DepthLayoutLoss
...
def one_epoch_full(model, criterion, layout_criterion, optimizer, config, dataloader, device, epoch, n_iters_total=0, is_train=True, lr=None, mean_and_std=None, limb_length=None, caption='', master=False, experiment_dir=None, writer=None, whole_val_dataloader=None, dist_size=None):
    ...
    with grad_context():
        ...
        while batch is not None:
            ...
            if (not is_train) and config.val.flip_test:
                keypoints_3d_pred, rel_pred, depth_graph = model(images_batch[:, 0], keypoints_2d_batch_cpn[:, 0], keypoints_2d_batch_cpn_crop[:, 0].clone(), depth_images_batch[:, 0])
                ...
                keypoints_3d_pred, rel_pred, depth_graph = model(images_batch, keypoints_2d_batch_cpn, keypoints_2d_batch_cpn_crop, depth_images_batch)
            ...
            loss_layout = layout_criterion(rel_pred, keypoints_3d_gt)
            lambda_layout = getattr(config.loss, "lambda_layout", 0.01)

            total_loss += (loss + lambda_layout * loss_layout)
            metric_dict[config.loss.criterion].append(loss.item())
            metric_dict['layout_loss'].append(loss_layout.item())
...
def main(args):
    ...
    model = DepthGuidedPoseLDSR(config, device)

    # experiment
    experiment_dir, writer = None, None
    if master:
        experiment_dir, writer = setup_experiment(config, type(model).__name__, is_train=not args.eval)
        shutil.copy('mvn/models/DGPose_ldsr.py', experiment_dir)
        shutil.copy('mvn/models/DGLifting_ldsr.py', experiment_dir)
        shutil.copy('train_ldsr.py', experiment_dir)
    ...
    layout_criterion = DepthLayoutLoss().to(device)
    ...
    if not args.eval:
        ...
        for epoch in range(config.train.n_epochs):
            ...
            epoch_loss_3d_train = one_epoch_full(model, criterion, layout_criterion, optimizer, config, train_dataloader, device, epoch, n_iters_total=n_iters_total_train, is_train=True, lr=lr_dict, master=master, experiment_dir=experiment_dir, writer=writer)
            result = one_epoch_full(model, criterion, layout_criterion, optimizer, config, val_dataloader, device, epoch, n_iters_total=n_iters_total_val, is_train=False, master=master, experiment_dir=experiment_dir, writer=writer, whole_val_dataloader=whole_val_dataloader, dist_size=dist_size)
        ...
        result = one_epoch_full(model, criterion, layout_criterion, None, config, val_dataloader, device, None, n_iters_total=0, is_train=False, master=master, experiment_dir=experiment_dir, writer=writer, whole_val_dataloader=whole_val_dataloader, dist_size=dist_size)
