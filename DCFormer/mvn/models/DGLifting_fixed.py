...
class UDEBlock(nn.Module):
    """
    Uncertainty-aware Depth Enhancer (UDE) Block.
    Strictly follows Algorithm 2 in the paper:
        Fcat_d = [Fd; DEmbed(mu); UMap(s)]
        F_attn = MSA(LN(Fcat_d)) + Fcat_d
        Fe_d   = MLP(LN(F_attn)) + F_attn
    Output is projected back to dim via out_proj.
    """
    def __init__(self, dim, num_heads=8, mlp_ratio=2., drop_path=0.):
        ...
        self.attn = nn.MultiheadAttention(concat_dim, num_heads, batch_first=True)
        self.drop_path1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        
        self.norm2 = nn.LayerNorm(concat_dim)
        mlp_hidden = int(concat_dim * mlp_ratio)
        self.mlp = Mlp(in_features=concat_dim, hidden_features=mlp_hidden,
                       out_features=concat_dim, act_layer=nn.GELU, drop=0.)
        self.drop_path2 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        
        # Project enhanced feature back to original dimension
        self.out_proj = nn.Linear(concat_dim, dim)
        
    def forward(self, fcat):
        # fcat: [B, J, 3*dim]
        attn_out, _ = self.attn(self.norm1(fcat), self.norm1(fcat), self.norm1(fcat))
        fcat = fcat + self.drop_path1(attn_out)
        
        fcat = fcat + self.drop_path2(self.mlp(self.norm2(fcat)))
        
        out = self.out_proj(fcat)
        return out
...
class DGLifting(nn.Module):
    def __init__(self, config=None, num_frame=1, num_joints=17, in_chans=2,
        ...
        self.ude_block = UDEBlock(
            dim=embed_dim_ratio, num_heads=num_heads, mlp_ratio=mlp_ratio, drop_path=0.)
    ...
    def forward(self, keypoints_2d, ref, depth_images, features_list_hr):
        ...
        x_depth = x[:, -1]  # [b, 17, 128]
        coarse_depth, uncer = self.depth_uncer(x_depth)  # [b, 17, 1], [b, 17, 1]

        # DEmbed(mu): depth embedding with positional encoding
        z_value = self.z_embed(coarse_depth) + self.Spatial_pos_embed2  # [b, 17, 128]

        # UMap(s): uncertainty score map via linear projection + softmax (feature-wise)
        umap = F.softmax(self.attn_fc(uncer), dim=-1)  # [b, 17, 128]

        # Fcat_d = [Fd; DEmbed(mu); UMap(s)]  (Algorithm 2)
        fcat = torch.cat([x_depth, z_value, umap], dim=-1)  # [b, 17, 128*3]

        # UDE Block: Pre-Norm MSA + Residual -> MLP + Residual -> Projection
        x_depth = self.ude_block(fcat)  # [b, 17, 128]

        # Replace original depth token with enhanced depth feature
        x = torch.cat((x[:, :-1], x_depth.unsqueeze(1)), dim=1)  # [b, 6, 17, 128]
