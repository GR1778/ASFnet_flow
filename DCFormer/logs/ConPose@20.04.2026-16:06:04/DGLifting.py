...
class DGLifting(nn.Module):
    def __init__(self, config=None, num_frame=1, num_joints=17, in_chans=2,
        ...
        self.depth_embed = nn.Conv2d(in_channels=1, out_channels=embed_dim_ratio, kernel_size=3, padding=1)  # [b, 1, 256, 192] -> [b, embed_dim_ratio, 256, 192],为什么不用线性层了？因为线性层只能处理固定大小的输入，而卷积层可以处理不同大小的输入，并且能够更好地捕捉空间信息，这对于处理深度图像来说是非常重要的。卷积层通过滑动窗口的方式在输入图像上进行特征提取，可以有效地捕捉局部空间结构和模式，从而提高模型对深度信息的理解和利用能力。此外，卷积层还具有参数共享的优势，可以减少模型的参数数量，提高训练效率。因此，在处理深度图像时，使用卷积层通常比使用线性层更合适。
        ...
        self.depth_uncer = DepthUncertaintyModel(embed_dim_ratio)    #输出每个关节的深度值和不确定性值
    ...
    def forward(self, keypoints_2d, ref, depth_images, features_list_hr):
        ...
        depth_images = self.depth_embed(depth_images.unsqueeze(1)) # [b, dim, 256, 192]   #这是将输入的深度图像通过卷积层进行特征提取，得到一个新的特征图，其中每个像素位置的特征维度为embed_dim_ratio，这些特征包含了原始深度信息以及通过卷积层提取的空间特征，可以更好地捕捉关节之间的空间关系和结构信息，从而提高3D姿态估计的准确性和鲁棒性。
        ...
        x_depth = x[:,-1]      #这是AMS特征提取模块一共五层，取最后一层的输出的深度特征 [b, 17, 128]
        coarse_depth, uncer = self.depth_uncer(x_depth)  #输出每个关节的深度值和不确定性值 [b, 17, 1]，[b, 17, 1]

        z_value = self.z_embed(coarse_depth) + self.Spatial_pos_embed2   #z_valuue是将深度值进行线性变换后加上一个位置编码，一个带位置编码的线性投影，将输入映射为具有空间感知的特征嵌入
    
        joint_uncer = F.softmax(self.attn_fc(uncer), dim=1)    # joint_uncer是对uncer进行线性变换后进行softmax得到的权重，用于在深度特征和z_value之间进行加权融合，用于生成不确定性分数图
        # x_depth = x_depth * (1-joint_uncer) + z_value * joint_uncer
        x_depth = torch.cat([joint_uncer,z_value,x_depth], dim=-1) #[b, 17, 128*3]     拼接后的特征包含了原始深度特征、位置编码后的深度特征以及不确定性权重，这些信息将被送入注意力机制进行融合，以生成更准确的深度估计和不确定性分数图
        x_depth = self.attn_depth(x_depth) #[b, 17, 128]     #通过注意力机制融合后的深度特征，结合了原始深度信息、位置编码后的深度信息以及不确定性权重，可以更好地捕捉关节之间的关系和空间结构，从而提高3D姿态估计的准确性和鲁棒性
        x = torch.cat((x[:, :-1], x_depth.unsqueeze(1)), dim=1)   #[b, 6, 17, 128]     将融合后的深度特征重新拼接回原始特征序列中，替换掉原来的深度特征位置，使得后续的Transformer模块能够利用融合后的深度信息进行更有效的特征提取和建模，从而提升3D姿态估计的性能
