...
class PoseSpaceDepthAligner(nn.Module):
    def __init__(self, dim=128, num_joints=17, num_heads=4, num_anchors=7, relation_temperature=1.5):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
    ...
    def forward(self, x_depth, x_pose, x_rgb, ref_2d):
        b, j, c = x_depth.shape
        root_xy = ref_2d[:, :1]
        ...
        h = h + self.pose_proj(x_pose) + self.rgb_proj(rgb_ctx) + self.xy_proj(rel_xy)
        ...
        z_delta = zi - zj
        pair_feat = torch.cat([h_i - h_j, h_i * h_j, z_delta.unsqueeze(-1), xy_i - xy_j], dim=-1)
        rel_res = self.rel_residual(pair_feat).squeeze(-1)
        rel_logits = self.relation_temperature * z_delta + rel_res - rel_res.transpose(1, 2)
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = (attn + relation_bias + topology_bias).softmax(dim=-1)
        ...
        x_depth = x_depth + gate * self.update_mlp(self.update_norm(msg))
        return x_depth, rel_pred, z_anchor
...
class DGLifting(nn.Module):
    def __init__(self, config=None, num_frame=1, num_joints=17, in_chans=2,
        ...
        self.depth_align = PoseSpaceDepthAligner(
            dim=embed_dim_ratio,
    ...
    def forward(self, keypoints_2d, ref, depth_images, features_list_hr):
        ...
        x_pose = x[:, 0]
        x_rgb = x[:, 1:-1]
        x_depth, rel_pred, z_anchor = self.depth_align(x_depth, x_pose, x_rgb, ref)
