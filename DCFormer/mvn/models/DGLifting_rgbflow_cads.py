...
class CenterAnchoredMotionDCEBlock(nn.Module):
    def __init__(self, dim, num_heads=4, num_samples=4, qkv_bias=True, drop_path=0.0, mlp_ratio=2):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.sample_embed = nn.Linear(dim, dim)
    ...
    def _reset_parameters(self):
        ...
        base = torch.stack([thetas.cos(), thetas.sin()], -1)
        base = 0.01 * (base / base.abs().max(-1, keepdim=True)[0])
        base = base.view(self.num_heads, 1, 2).repeat(1, self.num_samples, 1)
        for idx in range(self.num_samples):
            base[:, idx, :] *= idx + 1
        with torch.no_grad():
            self.sampling_offsets.bias = nn.Parameter(base.view(-1))

    def forward(self, flow_token, joint_query, ref, motion_field):
        b, p, c = flow_token.shape
        query = self.query_norm(flow_token + joint_query)

        offsets = self.sampling_offsets(query).view(b, p, self.num_heads, self.num_samples, 2).tanh()
        deform_pos = offsets + ref.view(b, p, 1, 1, 2)
        center_pos = ref.view(b, p, 1, 1, 2).expand(b, p, self.num_heads, 1, 2)
        pos = torch.cat([center_pos, deform_pos], dim=3)
        samples_per_head = self.num_samples + 1

        grid = pos.reshape(b, p * self.num_heads * samples_per_head, 1, 2)
        ...
        samples = samples.view(b, p, self.num_heads, samples_per_head, c)
        samples = self.sample_norm(self.sample_embed(samples))

        q = self.q_proj(query).view(b, p, self.num_heads, self.head_dim)
        k = self.k_proj(samples).view(b, p, self.num_heads, samples_per_head, self.num_heads, self.head_dim)
        v = self.v_proj(samples).view(b, p, self.num_heads, samples_per_head, self.num_heads, self.head_dim)

        head_idx = torch.arange(self.num_heads, device=flow_token.device)
        head_idx = head_idx.view(1, 1, self.num_heads, 1, 1, 1)
        head_idx = head_idx.expand(b, p, self.num_heads, samples_per_head, 1, self.head_dim)
...
class CenterAnchoredMotionDCE(nn.Module):
    def __init__(self, dim, depth, num_heads=4, num_samples=4, qkv_bias=True, drop_path=None):
        super().__init__()
        drop_path = drop_path or [0.0] * depth
        self.blocks = nn.ModuleList(
            [
                CenterAnchoredMotionDCEBlock(
                    dim=dim,
                    num_heads=num_heads,
...
class RGBFlowCADSLifting(nn.Module):
    def __init__(
        self,
        config=None,
        backbone="hrnet_32",
        ...
        flow_num_samples = getattr(config, "flow_num_samples", 4)
        out_dim = 3
        ...
        if backbone in ["hrnet_32", "hrnet_48"]:
            ...
            raise ValueError("Unsupported backbone for RGBFlowCADSLifting: {}".format(backbone))
        ...
        self.Motion_Extraction = CenterAnchoredMotionDCE(
            dim=embed_dim_ratio,
            depth=depth,
