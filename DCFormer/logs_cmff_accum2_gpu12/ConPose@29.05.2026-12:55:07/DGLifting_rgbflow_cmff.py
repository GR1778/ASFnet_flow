...
class RGBFlowCMFFLifting(nn.Module):
    ...
    def forward(self, keypoints_2d, ref, flow_images, features_list_hr):
        ...
        for level_idx in range(self.rgb_levels):
            stride = 2 ** level_idx
            if stride > 1:
                flow_features_level = F.avg_pool2d(flow_features, kernel_size=stride, stride=stride)
                flow_map_level = F.avg_pool2d(flow_map, kernel_size=stride, stride=stride)
            else:
                flow_features_level = flow_features
                flow_map_level = flow_map
