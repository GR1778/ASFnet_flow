...
class FlowFeatureEncoder(nn.Module):
    """Encode raw optical flow into a learned motion feature map."""
    ...
    def __init__(self, dim, num_layers=2):
        ...
        self.skip = nn.Conv2d(2, dim, kernel_size=1)
    ...
    def forward(self, flow):
        x = self.stem(flow) + self.skip(flow)
...
class RGBFlowAFFSLifting(nn.Module):
    def __init__(
        ...
        flow_encoder_layers = getattr(config, "flow_encoder_layers", 2)
        out_dim = 3
    ...
    def forward(self, keypoints_2d, ref, flow_images, features_list_hr):
        ...
        flow_feature_map = self.flow_feat_embed(flow_feature_map.permute(0, 2, 3, 1))
        flow_feature_map = flow_feature_map.permute(0, 3, 1, 2).contiguous()
