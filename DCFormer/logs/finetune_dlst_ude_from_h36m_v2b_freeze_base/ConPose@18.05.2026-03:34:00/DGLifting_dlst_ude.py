"""DGLifting variant: DLST -> RSDE in sequence.

The depth-joint token coming out of ``RGBD_Extraction`` is first organized by
the Depth-Layer Sorting Transformer (DLST), then refined by the Residual-
Shrinkage Depth Enhancer (RSDE). RSDE keeps the original UDE depth mean /
uncertainty supervision interface, but changes depth feature enhancement from
direct token regeneration to uncertainty-aware residual correction.
"""
...
class ResidualShrinkageDepthEnhancer(nn.Module):
    """Residual-Shrinkage Depth Enhancer (RSDE).

    This module replaces the post-DLST UDE block while keeping the same
    auxiliary outputs ``(coarse_depth, uncer)`` for the existing BNNLoss.

    Original UDE directly maps ``[UMap(s), DEmbed(mu), depth_token]`` to a new
    enhanced token. RSDE instead predicts an enhancement residual and applies
    adaptive soft-shrinkage before adding it back to the DLST token. This keeps
    the module boundary clear: DLST performs inter-joint depth-layer sorting,
    while RSDE performs uncertainty-aware residual refinement of the resulting
    depth token.
    """
    ...
    def __init__(
        ...
        # Keep UDE's depth mean / log-variance prediction interface.
        self.depth_uncer = DepthUncertaintyModel(dim)
        self.Spatial_pos_embed2 = nn.Parameter(torch.zeros(1, num_joints, dim))
        self.z_embed = nn.Linear(1, dim)
        self.attn_fc = nn.Linear(1, dim)

        # Candidate residual generator. Input format is the same as UDE:
        # [UMap(s), DEmbed(mu), depth_token]. Output is a residual, not a
        # full replacement token.
        self.residual_attn = Attention(
            dim=dim * 3,
            dim_out=dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
        ...
        # Adaptive soft-threshold predictor. The threshold ratio is estimated
        # from the learned depth and uncertainty embeddings. This is not a
        # separate loss or extra config; it is the residual-selection operator.
    ...
    def forward(self, x_depth):
        ...
        Args:
            x_depth: Tensor of shape [B, J, C], usually the DLST output token.

        Returns:
            x_out: Enhanced depth token, [B, J, C].
            coarse_depth: Predicted depth mean, [B, J, 1].
            uncer: Predicted log-variance, [B, J, 1].
        """
        coarse_depth, uncer = self.depth_uncer(x_depth)
        uncer = torch.clamp(uncer, min=-9.0, max=9.0)

        z_value = self.z_embed(coarse_depth) + self.Spatial_pos_embed2
        joint_uncer = F.softmax(self.attn_fc(uncer), dim=1)

        x_cat = torch.cat([joint_uncer, z_value, x_depth], dim=-1)
        residual = self.residual_attn(x_cat)

        # Adaptive residual shrinkage. The residual magnitude determines the
        # threshold scale, and the learned ratio decides how much correction is
        # retained for each channel and joint.
        shrink_ratio = self.threshold(torch.cat([z_value, joint_uncer], dim=-1))
        residual_scale = residual.abs().mean(dim=-1, keepdim=True)
        threshold = shrink_ratio * residual_scale
        residual = self.soft_shrink(residual, threshold)

        x_out = self.out_norm(x_depth + residual)
        return x_out, coarse_depth, uncer
...
class DGLifting(nn.Module):
    """DLST -> RSDE sequential variant of DGLifting.

    The depth-joint token from RGBD_Extraction is first passed through DLST
    (depth-layer sorting transformer), then refined by RSDE. RSDE keeps the
    same ``(coarse_depth, uncer)`` output interface as UDE, so the existing
    BNN depth loss and 3-tuple unpacking in train.py still apply.
    ...
    def __init__(
        ...
        # ---- RSDE (Residual-Shrinkage Depth Enhancer) ----
        self.rsde = ResidualShrinkageDepthEnhancer(
            dim=embed_dim_ratio,
    ...
    def forward(self, keypoints_2d, ref, depth_images, features_list_hr):
        ...
        # ---- RSDE: refine the DLST-organized depth token by residual shrinkage ----
        x_depth, coarse_depth, uncer = self.rsde(x_depth)
