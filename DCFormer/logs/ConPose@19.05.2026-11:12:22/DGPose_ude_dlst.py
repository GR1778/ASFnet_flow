...
from mvn.models.DGLifting_ude_dlst import DGLifting


class DepthGuidedPoseUDEDLST(nn.Module):
    """Baseline (UDE) + DLST head: keeps UDE's BNN supervision and adds DLST's
    ordering supervision on top. Lifting net forward returns
    ``(keypoints_3d, (coarse_depth, uncer), (rel_depth, layer_assign))``.
    """
