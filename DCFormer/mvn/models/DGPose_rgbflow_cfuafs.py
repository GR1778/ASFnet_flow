...
class RGBFlowPoseCFUAFS(nn.Module):
    def __init__(self, config, device="cuda:0"):
        ...
        if config.model.backbone.type in ["hrnet_32", "hrnet_48"]:
            ...
            raise ValueError("RGBFlowPoseCFUAFS currently supports HRNet backbones only.")
        ...
        self.Lifting_net = RGBFlowCFUAFSLifting(
            config.model.poseformer,
            backbone=config.model.backbone.type,
            num_joints=self.num_joints,
        )

    def forward(self, images, keypoints_2d_cpn, keypoints_2d_cpn_crop, flow_images, keypoints_3d_gt=None):
        ...
        keypoints_3d, aux = self.Lifting_net(
            keypoints_2d_cpn,
