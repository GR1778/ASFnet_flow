"""DGLifting variant: DLST and UDE in parallel.

The depth-joint token coming out of ``RGBD_Extraction`` is sent to DLST and UDE
independently. Their outputs are concatenated with the raw depth token and
projected back to the original token dimension before the main fusion
transformer. This avoids the destructive overwrite behavior seen in sequential
DLST->UDE / UDE->DLST ablations.
"""
...
class DGLifting(nn.Module):
    """Parallel DLST + UDE variant of DGLifting.

    The depth-joint token from RGBD_Extraction is processed independently by
    DLST (depth-layer sorting transformer) and UDE (uncertainty-aware depth
    enhancer), then fused by concat + linear projection. UDE's ``coarse_depth``
    and ``uncer`` plus DLST's ``rel_depth`` and ``layer_assign`` are returned so
    the baseline BNN loss and DLST ordering loss can both be applied.
    ...
    def __init__(
        ...
        self.depth_fusion = nn.Linear(embed_dim_ratio * 3, embed_dim_ratio)
        self._init_depth_fusion(embed_dim_ratio)
    ...
    def _init_depth_fusion(self, dim):
        nn.init.zeros_(self.depth_fusion.weight)
        nn.init.zeros_(self.depth_fusion.bias)
        with torch.no_grad():
            self.depth_fusion.weight[:, dim : 2 * dim].copy_(torch.eye(dim))
    ...
    def forward(self, keypoints_2d, ref, depth_images, features_list_hr):
        ...
        x_depth_dlst, rel_depth, layer_assign = self.dlst(x_depth_raw)

        # ---- UDE: refine the same raw AMS depth tokens independently ----
        coarse_depth, uncer = self.depth_uncer(x_depth_raw)
        z_value = self.z_embed(coarse_depth) + self.Spatial_pos_embed2
        joint_uncer = F.softmax(self.attn_fc(uncer), dim=1)
        x_depth_ude = torch.cat([joint_uncer, z_value, x_depth_raw], dim=-1)
        x_depth_ude = self.attn_depth(x_depth_ude)

        # ---- Parallel fusion: raw + DLST + UDE -> original depth-token width ----
        x_depth = self.depth_fusion(torch.cat([x_depth_raw, x_depth_dlst, x_depth_ude], dim=-1))
