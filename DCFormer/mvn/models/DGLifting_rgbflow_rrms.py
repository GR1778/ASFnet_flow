...
class ReferenceRefinedMotionSampleBlock(nn.Module):
    def __init__(self, dim, num_heads=4, num_samples=5, qkv_bias=True, drop_path=0.0, mlp_ratio=2):
        ...
        self.reference_offsets = nn.Linear(dim, 2)
    ...
    def _reset_parameters(self):
        constant_(self.reference_offsets.weight.data, 0.0)
        constant_(self.reference_offsets.bias.data, 0.0)
    ...
    def forward(self, flow_token, joint_query, ref, motion_field):
        ...
        ref = (ref + 0.25 * self.reference_offsets(query).tanh()).clamp(-1.0, 1.0)
        offsets = self.sampling_offsets(query).view(b, p, self.num_heads, self.num_samples, 2).tanh()
        pos = offsets + ref.view(b, p, 1, 1, 2)
        ...
        samples = samples.view(b, p, self.num_heads, self.num_samples, c)
...
class ReferenceRefinedMotionSampling(nn.Module):
    ...
    def forward(self, flow_token, joint_query, ref, motion_field):
        for block in self.blocks:
            flow_token, ref = block(flow_token, joint_query, ref, motion_field)
        return flow_token


class RGBFlowRRMSLifting(nn.Module):
    def __init__(
        self,
        config=None,
        backbone="hrnet_32",
        ...
        if backbone in ["hrnet_32", "hrnet_48"]:
            ...
            raise ValueError("Unsupported backbone for RGBFlowRRMSLifting: {}".format(backbone))
        ...
        self.Motion_Extraction = ReferenceRefinedMotionSampling(
            dim=embed_dim_ratio,
            depth=depth,
